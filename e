...
-- Configuration dengan delay yang lebih cepat
local CONFIG = {
    AUTO_CLICK_DELAY = 0.1,  -- Dipercepat dari 0.5 ke 0.1
    CLICK_INTERVAL = 0.05,   -- Delay antara click down dan up
    DOUBLE_CLICK_DELAY = 0.08, -- Delay antara double click
    UPDATE_INTERVAL = 0.1,
    MIN_SAFE_CHANCE = 0.4,
    MAX_SAFE_CHANCE = 0.7,
...
-- Auto Play Functions dengan double click yang lebih cepat
...
local function simulateClick(button)
    ...
    local success = pcall(function()
        if VirtualInputManager then
            ...
            task.wait(CONFIG.CLICK_INTERVAL) -- Delay sangat singkat
            -- Click up
...
-- Fungsi double click yang lebih cepat
local function simulateDoubleClick(button)
    if not AutoAvailable then return false end
    ...
    local success = pcall(function()
        if VirtualInputManager then
            ...
            task.wait(CONFIG.DOUBLE_CLICK_DELAY)
    ...
    if success then
        STATE.SessionStats.Clicks = STATE.SessionStats.Clicks + 2
        updateStatsDisplay()
    end
    ...
    STATE.AutoLoopThread = task.spawn(function()
        while STATE.AutoOn and ScreenGui.Parent do
            ...
            if rec == "TAKE" and confidence > 60 then
                AutoStatus.Text = "Action: Taking card (Double Click)..."
                simulateDoubleClick("left") -- Gunakan double click yang cepat
            elseif rec == "HOLD" and confidence > 60 then
                AutoStatus.Text = "Action: Holding (Double Click)..."
                simulateDoubleClick("right") -- Gunakan double click yang cepat
            else
                AutoStatus.Text = "Waiting: Low confidence..."
            end
            
            task.wait(CONFIG.AUTO_CLICK_DELAY) -- Delay utama yang lebih pendek
        end
...
TakeButton.MouseButton1Click:Connect(function()
    ...
    simulateDoubleClick("left") -- Manual juga pakai double click
...
HoldButton.MouseButton1Click:Connect(function()
    ...
    simulateDoubleClick("right") -- Manual juga pakai double click
    STATE.GameStats.GamesPlayed = STATE.GameStats.GamesPlayed + 1
    updateStatsDisplay()
end)
...
warn("Advanced Card Counter loaded! Features: Fast Auto-play, Quick double clicks, Enhanced performance")
