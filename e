...
AutoButton.Size = UDim2.new(0, 30, 0, 25)
AutoButton.Position = UDim2.new(1, -60, 0, 2)
...
local function doClick(isLeftClick, times)
	if not VirtualInputManager and not VirtualUser then return end
	
	local button = isLeftClick and 0 or 1 
	local vx = Frame.AbsolutePosition.X + Frame.AbsoluteSize.X / 2
	local vy = Frame.AbsolutePosition.Y + Frame.AbsoluteSize.Y / 2
...
local function startAutoLoop()
	...
	AutoLoopThread = task.spawn(function()
		while AutoOn and ScreenGui.Parent do
			local rec = currentRecommendation
			
			if rec:find("HIT/TAKE") then
				doClick(true, 2) 
				task.wait(0.5) 
			elseif rec:find("STAND/HOLD") or rec:find("BLACKJACK/HOLD") then
				doClick(false, 2) 
				task.wait(5) 
			else
				task.wait(0.5) 
...
AutoButton.MouseButton1Click:Connect(function()
	if not AutoAvailable then
		...
		task.delay(0.5, function() setAutoAppearance(AutoOn) end)
		InfoLabel.Text = "<b>[ERROR AUTO]</b> Tidak ada metode klik yang didukung ditemukan."
		return
	end
